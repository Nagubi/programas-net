...
void Inicializar() {
    ...
    if(red is null) {
        ...
        red.NuevoNodo(new Nodo(ip:"192.168.0.10", tipo:"servidor", puertos:5, saltos:10, so:"Linux"));
        red.NuevoNodo(new Nodo(ip:"192.168.0.12", tipo:"equipoactivo", puertos:2, saltos:12, so:"ios"));
        red.NuevoNodo(new Nodo(ip:"192.168.0.20", tipo:"computadora", puertos:8, saltos:5, so:"Windows"));
        red.NuevoNodo(new Nodo(ip:"192.168.0.15", tipo:"servidor", puertos:10, saltos:22, so:"Linux"));

        red.Nodos[0].NuevaVulnerabilidad(new Vulnerabilidad(clave:"CVE-2015-1635", vendedor:"microsoft", 
        descripcion:"HTTP.sys permite a atacantes remotos ejecutar codigo arbitrario", tipo:"remota", fecha:DateTime.Parse("14/04/2015")));
        red.Nodos[0].NuevaVulnerabilidad(new Vulnerabilidad(clave:"CVE-2017-0004", vendedor:"microsoft", 
        descripcion:"El servicio LSASS permite causar una denegacion de servicio", tipo:"local", fecha:DateTime.Parse("10/01/2011")));
        red.Nodos[1].NuevaVulnerabilidad(new Vulnerabilidad(clave:"CVE-2017-3847", vendedor:"cisco", 
        descripcion:"Cisco Firepower Management Center XSS", tipo:"remota", fecha:DateTime.Parse("21/02/2017")));
        red.Nodos[2].NuevaVulnerabilidad(new Vulnerabilidad(clave:"CVE-2009-2504", vendedor:"microsoft", 
        descripcion:"Multiples desbordamientos de enteros en APIs Microsoft .NET 1.1", tipo:"local", fecha:DateTime.Parse("13/11/2009")));
        red.Nodos[2].NuevaVulnerabilidad(new Vulnerabilidad(clave:"CVE-2016-7271", vendedor:"microsoft", 
        descripcion:"Elevacion de privilegios Kernel Segura en Windows 10 Gold", tipo:"local", fecha:DateTime.Parse("20/12/2016")));
        red.Nodos[2].NuevaVulnerabilidad(new Vulnerabilidad(clave:"CVE-2017-2996", vendedor:"adobe", 
...
void NuevaVulnerabilidad() {
    ...
    if(red is not null) {
        ...
        if(red.Nodos.Count != 0) {
            ...
            red.Nodos[op].NuevaVulnerabilidad(vulnerabilidad);
            Console.WriteLine("\nVulnerabilidad agregada");
            red.Nodos[op].Vulnerabilidades.ForEach(p => Console.WriteLine(p.ToString()));
        } else {Console.WriteLine("\nNo hay nodos para agregar una vulnerabilidad");}
    } else {Console.WriteLine("\nNo hay datos en memoria");}
}
...
do {
    ...
    switch(op) {
        ...
        case 2 : NuevoNodo(); break;
        case 3 : NuevaVulnerabilidad(); break;
